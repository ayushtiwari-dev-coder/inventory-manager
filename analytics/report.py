...
class analytics:
    ...
    def top_products_by_profit(org_id, limit=10):
        # Line-item level analytics still requires the raw tables, which is fine
        query = """
            SELECT 
                p.product_name,
    ...
    def revenue_summary(org_id, period=None):
        # Now querying the highly optimized daily_summaries table
        if period == "daily":
            date_filter = "AND summary_date=CURDATE()"
        elif period == "weekly":
            date_filter = "AND WEEK(summary_date)=WEEK(CURDATE()) AND YEAR(summary_date)=YEAR(CURDATE())"
        elif period == "monthly":
            date_filter = "AND MONTH(summary_date)=MONTH(CURDATE()) AND YEAR(summary_date)=YEAR(CURDATE())"
        elif period == "3monthly":
            date_filter = "AND summary_date >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH)"
        elif period == "yearly":
            date_filter = "AND YEAR(summary_date)=YEAR(CURDATE())"
        else:
            date_filter = ""
    ...
    def sales_trend(org_id, months=4):
        # O(1) query. No GROUP BY needed because the table is already 1 row per day!
        query = """
            SELECT 
            ...
            FROM daily_summaries
            WHERE org_id = %s
            AND summary_date >= DATE_SUB(CURDATE(), INTERVAL %s MONTH)
